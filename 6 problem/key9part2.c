...
void sum(int buff1[], int len1, int buff2[], int len2, int result[],
         int *result_length) {
  if (len1 - len2 > 0)
    make_cycle_shift(buff2, REAL_LEN, len1 - len2);
  if (len2 - len1 > 0)
    make_cycle_shift(buff1, REAL_LEN, len2 - len1);
  int d = 0;
...
int main() {
  ...
  if (num1_is_error || num2_is_error) {
    ...
    init_num(sum_num);
    sum(num1, num1_digit_cnt, num2, num2_digit_cnt, sum_num,
        &sum_num_digit_cnt);
    ...
    init_num(sub_num);
    if (sub(num1, num1_digit_cnt, num2, num2_digit_cnt, sub_num,
            &sub_num_digit_cnt) == 2) {
      printf("n/a");
    } else {
